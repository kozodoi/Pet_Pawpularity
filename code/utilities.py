...
def get_score(y_true, y_pred):
    score = np.sqrt(mean_squared_error(y_true * 100, y_pred * 100))
    return score

# image paths
def get_train_file_path(image_id, CFG):
    return CFG['data_path'] + 'train/{}.jpg'.format(image_id)
def get_test_file_path(image_id, CFG):
    return CFG['data_path'] + 'test/{}.jpg'.format(image_id)
def get_old_file_path(image_id, CFG):
    return CFG['data_path'] + 'old/{}-1.jpg'.format(image_id)
